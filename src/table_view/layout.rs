use super::table::CellDecoration;
...
impl<'l> StripLayout<'l> {
    ...
    pub(crate) fn add(
        ...
        if let Some(color) = decoration.bg_color {
            self.ui.painter().rect_filled(stripe_rect, 0.0, color);
        }
        if let Some(color) = decoration.inner_shadow {
            super::widgets::draw_shadow_rect(self.ui, stripe_rect, 10.0, color);
        }
