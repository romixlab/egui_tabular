...
use egui::{Align, Color32, NumExt as _, Rect, Response, ScrollArea, Ui, Vec2};
use log::error;
use std::ops::RangeInclusive;

use super::layout::{CellDirection, CellSize, StripLayout};
use super::util::blue_shift_color;
...
fn to_sizing(columns: &[Column]) -> super::sizing::Sizing {
    use super::sizing::Size;

    let mut sizing = super::sizing::Sizing::default();
...
/// Used when multiple cells are selected, editor is not shown then, but can copy/paste blocks.
...
impl SelectedRange {
    ...
    pub fn ordered(
        ...
        SelectedRange {
            ...
            col_start: mono_col_start.min(mono_col_end),
            row_end: mono_row_start.max(mono_row_end),
            col_end: mono_col_start.max(mono_col_end),
        }
    }
...
impl<'a> TableBuilder<'a> {
    ...
    pub fn select_range(mut self, range: Option<SelectedRange>) -> Self {
...
impl<'a> Table<'a> {
    ...
    pub fn body<F>(self, add_body_contents: F) -> Vec<f32>
    where
        F: for<'b> FnOnce(TableBody<'b>),
    {
        let Table {
    ...
    pub fn set_column_widths(&mut self, width: impl Iterator<Item = f32>) {
        for (i, w) in width.enumerate() {
            if w < 0.1 {
                return;
            }
            self.state.column_widths[i] = w;
        }
    }
...
impl<'a> TableBody<'a> {
    ...
    pub fn rows(
        ...
        // let enable_hover_and_clicks = self.layout.ui.ui_contains_pointer();
        let (pointer_hover_pos, pointer_pressed, pointer_released) = if enable_hover_and_clicks {
            self.layout.ui.ctx().input(|i| {
                (
                    i.pointer.hover_pos(),
        ...
        let col_hovered = pointer_hover_pos.and_then(|pointer| {
            let mut col_x_start = self.layout.cursor.x;
        ...
        for idx in min_row..max_row {
            let row_hovered = if enable_hover_and_clicks {
                let row_y_start = self.layout.cursor.y;
                pointer_hover_pos
                    .map(|pointer| {
                        pointer.y >= row_y_start
                            && pointer.y <= row_y_start + row_height_with_spacing
                    })
                    .unwrap_or(false)
            } else {
                false
            };
            let cols_selected = self.selected_range.and_then(|r| {
                if idx >= r.row_start && idx <= r.row_end {
                    Some(RangeInclusive::new(r.col_start, r.col_end))
                } else {
                    None
                }
            });

            if let Some(col) = col_hovered {
                if row_hovered && pointer_pressed {
                    selection_event = Some(SelectionEvent::Pressed(idx, col));
                }
                if row_hovered && pointer_released {
                    selection_event = Some(SelectionEvent::Released(idx, col));
                }
            }
            ...
            add_row_content(
                ...
                TableRow {
                    ...
                    row_hovered,
                    col_hovered: col_hovered.filter(|_| row_hovered),
                    cols_selected,
...
pub struct CellDecoration {
    ...
    pub inner_shadow: Option<Color32>,
}
...
impl<'a, 'b> TableRow<'a, 'b> {
    ...
    pub fn col(&mut self, add_cell_contents: impl FnOnce(&mut Ui)) -> (Rect, Response) {
        ...
        let bg_color = self.bg_color(
            self.striped,
            self.row_hovered,
            self.col_hovered == Some(col_index),
        );
        let inner_shadow = self.cols_selected.as_ref().and_then(|r| {
            if r.contains(&col_index) {
                Some(Color32::DARK_BLUE)
            } else {
                None
            }
        });
        ...
        let (used_rect, response) =
            self.layout
                .add(clip, decoration, width, height, add_cell_contents);
        // if self.layout.ui.ctx().input(|i| {
        //     i.pointer
        //         .hover_pos()
        //         .map(|pointer| max_rect.contains(pointer))
        //         .unwrap_or(false)
        //         && i.pointer.primary_clicked()
        // }) {
        //     debug!("clicked {col_index}");
        // }
        // if col_index == 0 {
        //     // let r = Rect::from_two_pos()
        //     // debug!("clip rect: {r:?}");
        //     let p = self.layout.ui.painter();
        //     p.rect_stroke(
        //         max_rect,
        //         Rounding::default(),
    ...
    fn bg_color(&self, striped: bool, row_hovered: bool, col_hovered: bool) -> Option<Color32> {
        let faint_bg_color = self.layout.ui.visuals().faint_bg_color;
        match (striped, row_hovered, col_hovered) {
            (false, false, false) => None,
            (true, true, false) => Some(blue_shift_color(faint_bg_color, 15)),
            (false, true, false) => Some(blue_shift_color(faint_bg_color, 20)),
            (true, false, false) => Some(faint_bg_color),
            (_, true, true) => Some(blue_shift_color(faint_bg_color, 40)),
            _ => None
            // (false, _, true) => Some(Color32::DARK_BLUE),
            // (true, _, true) => Some(Color32::DARK_BLUE),
        }
    }
